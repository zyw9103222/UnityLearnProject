...
namespace FarmingEngine
{
    ...
    public class PlayerCharacterData
    {
        ...
        public Vector3Data position; // 位置
        public int gold = 0; // 金币

        public Dictionary<string, PlayerLevelData> levels = new Dictionary<string, PlayerLevelData>(); // 等级数据
        public Dictionary<AttributeType, float> attributes = new Dictionary<AttributeType, float>(); // 属性值
        public Dictionary<BonusType, TimedBonusData> timed_bonus_effects = new Dictionary<BonusType, TimedBonusData>(); // 时限奖励效果
        public Dictionary<string, int> crafted_count = new Dictionary<string, int>(); // 制作计数
        public Dictionary<string, int> kill_count = new Dictionary<string, int>(); // 击杀计数
        public Dictionary<string, bool> unlocked_ids = new Dictionary<string, bool>(); // 解锁的ID
        public Dictionary<string, PlayerPetData> pets = new Dictionary<string, PlayerPetData>(); // 玩家拥有的宠物数据

        public PlayerCharacterData(int id) { player_id = id; }

        public void FixData()
        {
            // 修复数据以确保旧的保存文件与新游戏版本兼容
            if (levels == null)
